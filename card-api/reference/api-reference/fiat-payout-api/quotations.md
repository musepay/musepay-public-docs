...
| pay_amount | Number | Conditional | Amount deducted from the USDT balance. Required when `quote_mode` is `source`. |
...
| receive_amount | Number | Conditional | Fiat amount received by the beneficiary. Required when `quote_mode` is `dest`. |
...
| exchange_rate | Number | Exchange rate from `pay_currency` to `receive_currency`. |
